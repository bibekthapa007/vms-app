...
export const fetchVaccines = createAsyncThunk('vaccine/list', async (data, thunkApi) => {
  try {
...
export const createVaccine = createAsyncThunk(
  ...
  async (vaccine: VaccineForm, thunkApi) => {
    try {
      const token = localStorage.getItem('token');
      console.log(vaccine, vaccine.userFiles[0], 'Form create Vaccine');
      const data = new FormData();
      data.append('file', vaccine.userFiles[0]);
      data.append('name', vaccine.name);
      data.append('description', vaccine.description);
      data.append('is_mandatory', JSON.stringify(vaccine.is_mandatory || false));
...
export const editVaccine = createAsyncThunk('vaccine/edit', async (vaccine: Vaccine, thunkApi) => {
  try {
    const token = localStorage.getItem('token');
...
export const vaccineSlice = createSlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(fetchVaccines.fulfilled, (state, action: PayloadAction<VaccinesResponse>) => {
      ...
      .addCase(fetchVaccine.fulfilled, (state, action: PayloadAction<VaccineResponse>) => {
      ...
      .addCase(createVaccine.fulfilled, (state, action: PayloadAction<VaccineResponse>) => {
      ...
      .addCase(editVaccine.fulfilled, (state, action: PayloadAction<VaccineResponse>) => {
        state.editing = false;
      ...
      .addCase(deleteVaccine.fulfilled, (state, action: PayloadAction<VaccineResponse>) => {
